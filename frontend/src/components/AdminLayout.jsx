  ...
  return (
    <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 py-10 flex flex-col gap-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h2 className="font-serif text-2xl font-bold text-brand-950">Admin Dashboard</h2>
          <p className="text-sm text-brand-700/60">Signed in as {username}</p>
        </div>
        <button
          type="button"
          onClick={handleLogout}
          className="text-sm text-brand-700 hover:text-gold underline underline-offset-2 transition-colors"
        >
          Sign out
        </button>
      </div>

      <nav className="flex gap-2 border-b border-brand-200 pb-3">
